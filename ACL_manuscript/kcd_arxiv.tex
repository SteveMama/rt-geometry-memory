...
On Multi-Session Conversations (50 conversations, Qwen2.5-1.5B-Instruct),
\semqcg{} achieves logit $\ell_2$ of $636.4$ at budget $0.50$---lower
than uniform sampling ($682.8$)---while simultaneously improving answer NLL
versus both semantic and geometry variants
($\Delta{-}113.7$ logit $\ell_2$ and $\Delta{-}0.056$ NLL versus \semkcd{},
...
Most prior work treats this as a single-objective problem: minimize the loss
on held-out continuations, or equivalently, preserve behavioral quality
as measured by answer NLL \citep{jiang-etal-2024-longllmlingua, pan-etal-2024-llmlingua2}.
...
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt]
  ...
  \item We characterize two \textbf{scope conditions}---architecture-dependence
    (Llama-3.2-3B reversal) and length-dependence (400--600 turn conversations)---
...
This compact structure is consistent across model families and
motivates transported geometry as a discriminative signal.
...
a reference point $\hv_{\text{ref}}$ (taken as the mean of the trajectory).
...
($n{=}50$ conversations, Qwen2.5-1.5B).
...
\geokcd{} & 709.8 & 724.4 & 737.0 & 3.201 & 3.052 & 2.923 \\
\semkcd{} & 713.4 & 740.0 & 750.1 & 3.176 & 2.993 & 2.845 \\
\semqcg{} & 717.8 & \textbf{723.0} & \textbf{636.4} & \textbf{3.143} & \textbf{2.944} & \textbf{2.789} \\
\bottomrule
\end{tabular}
\end{table}
...
on MSC valid ($n{=}50$ conversations).
...
\midrule
$0.20$ & $+7.88$ & $0.092$ & $+4.45$ & $0.296$ & $-0.034$ & $0.012$ \\
$0.35$ & $-14.53$ & $0.014$ & $-17.05$ & $\mathbf{0.005}$ & $-0.049$ & $\mathbf{0.003}$ \\
$0.50$ & $-114.49$ & $\mathbf{<}0.001$ & $-113.70$ & $\mathbf{<}0.001$ & $-0.056$ & $\mathbf{0.004}$ \\
\bottomrule
\end{tabular}
\end{table}
...
At budget $0.50$, \semqcg{} achieves logit $\ell_2 = 636.4$---\emph{lower
than uniform sampling} ($682.8$)---meaning the compressed context is more
faithful to the full-context hidden state than randomly retaining half the turns.
This is the only regime in our experiments where a compression policy
is strictly more geometrically faithful than the no-selection baseline.
...
$\Delta{-}17.05$ logit $\ell_2$ ($p_\text{conv} = 0.005$) and the behavioral
advantage is $\Delta{-}0.049$ NLL ($p_\text{conv} = 0.003$).
At $B{=}0.50$ the geometry improvement is dramatic
($\Delta{-}113.70$, $p_\text{conv} < 0.001$)
with 95\% CI $[-142.3, -87.0]$.
...
We attribute this to differences in pretraining regime:
Qwen2.5 models undergo heavy instruction-following finetuning
that produces discriminative per-turn geometry---user instructions
reliably move the hidden state more than assistant echoes.
Llama-3.2-3B's broader text training does not produce this property:
...
The scope condition is therefore about model representation structure,
not parameter count.
...
400--600 turns, Qwen2.5-1.5B). Lower is better.}
\label{tab:lme}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lcccccc}
\toprule
...
\geokcd{} & 1751.0 & 1704.5 & \textbf{1692.4} & 0.123 & 0.118 & 0.114 \\
\semkcd{} & \textbf{1740.4} & \textbf{1693.6} & 1714.2 & \textbf{0.114} & \textbf{0.112} & \textbf{0.112} \\
\semqcg{} & 1740.7 & 1743.9 & 1696.9 & 0.124 & 0.122 & 0.112 \\
\bottomrule
\end{tabular}
\end{table}
...
At budget $0.35$, \semqcg{} is significantly \emph{worse} than \semkcd{}
on logit $\ell_2$: $\Delta{+}50.3$, $p_\text{conv} = 0.011$.
...
(MSC regime).
For very long conversations ($>400$ turns), semantic similarity
is the more stable selection criterion.
Signal design should be conditioned on expected conversation length.
...
\citet{nawrot-etal-2024-dynamic} introduce dynamic memory compression
for efficient LLM inference via key-value cache compression.
Our work differs in operating at the turn level rather than token or
key-value cache level, and in introducing dual-objective evaluation.
...
\citet{huh-etal-2024-platonic} observe convergence of representations across
model families toward a common geometric structure, motivating
cross-model geometric analysis.
\citet{xiao-etal-2023-efficient} identify attention sinks---tokens that
accumulate disproportionate attention mass---which we account for in our
hidden-state extraction via sink-corrected attention weights.
Recent work on intrinsic dimensionality of transformer representations
\citep{anagnostidis-etal-2022-signal} provides theoretical grounding for
the low Rank-95 values we observe ($1.05$--$1.53$).
To our knowledge, we are the first to use \emph{transported geometry}
...
\textbf{Hard stress set}: 9 synthetic conversations
(3 long-dependency, 3 retrieval-heavy, 3 code-conversation),
$540$ evaluation rows, hand-constructed by the authors.
Designed to maximize signal-to-noise difficulty for support-turn detection.
...
Budget & $\Delta$ L2 (row) & $p_\text{row}$ & $\Delta$ L2 (conv) & $p_\text{conv}$ & $\Delta$ NLL (conv) & $p_\text{conv}$ \\
\midrule
$0.20$ & $+0.26$  & $0.973$ & $+0.26$  & $0.988$ & $+0.010$ & $0.623$ \\
$0.35$ & $+50.31$ & ${<}0.001$ & $+50.31$ & $0.011$ & $+0.010$ & $0.819$ \\
$0.50$ & $-17.35$ & $0.025$ & $-17.35$ & $0.517$ & $\approx 0$ & $0.683$ \\
\bottomrule
\end{tabular}
\end{table}
