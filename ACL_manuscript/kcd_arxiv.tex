...
\semqcg{} & 807.5 & \textbf{801.8} & \textbf{705.5} & \textbf{3.034} & \textbf{2.850} & \textbf{2.671} \\
\bottomrule
\end{tabular}
\end{table}
...
Removing support-score weighting (\textit{w/o support}) degrades both metrics
at all budgets ($p{<}0.001$). Removing query projection (\textit{w/o query})
causes moderate geometry degradation at $B{=}0.35$ ($\Delta{=}{+}7.5$, $p{<}0.001$)
but negligible behavioral effect ($p{=}0.677$).}
...
Policy & $B{=}0.20$ & $B{=}0.35$ & $B{=}0.50$ & $B{=}0.20$ & $B{=}0.35$ & $B{=}0.50$ \\
\midrule
\semqcg{} & \textbf{807.5} & \textbf{801.8} & \textbf{705.5} & \textbf{3.034} & \textbf{2.850} & \textbf{2.671} \\
...
At budget $0.50$, \semqcg{} achieves logit $\ell_2 = 705.5$---\emph{lower
than uniform sampling} ($761.6$)---meaning the compressed context is more
...
Removing query-conditioned projection (\textit{w/o query}) causes moderate geometry
degradation at $B{=}0.35$ ($+7.5$ logit $\ell_2$, $p_\text{conv} < 0.001$) but
has negligible behavioral effect ($p_\text{conv} = 0.677$ at $B{=}0.50$),
indicating the query projection contributes primarily through geometric discriminability
...
on logit $\ell_2$: $\Delta{=}{+}47.6$, $p_\text{conv} = 0.018$, $p_\text{row} < 0.001$.
On behavior, \semkcd{} achieves the best NLL across all budgets
