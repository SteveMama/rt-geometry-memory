...
\title{Geometry-Conditioned Turn Scoring for Conversational Memory Compression}
...
\label{app:geometry}
Conversation-state trajectories are highly compact (rank-95 near 1.0) and geometric distortion closely tracks decoder drift (correlation $>0.989$). These values indicate that within-segment conversational motion is tightly coupled to decoder behavior, which motivates using transported geometry as a compression-control signal.
