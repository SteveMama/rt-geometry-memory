...
\begin{itemize}[leftmargin=1.25em]
    \item \textbf{Hard stress set}: 36 synthetic but interpretable conversations across long-dependency, retrieval-heavy, and code-conversation families, designed to expose support-turn failures that are plausible in real conversational memory settings. The packets are randomized synthetic instances rather than copied benchmark rows, but they are built from the same conversational memory primitives that recur in public datasets: personal facts, event details, task constraints, and formatting commitments of the kind seen in MSC, LoCoMo, and LongMemEval \cite{msc,locomo,longmemeval}.
    \item \textbf{MSC valid}: 32 conversations from the multi-session persona benchmark \cite{msc}.
    \item \textbf{LongMemEval-S cleaned}: a 12-conversation subset from the cleaned LongMemEval release \cite{longmemeval}, truncated to 40 turns per conversation for oracle tractability.
\end{itemize}
...
The public-benchmark oracle studies operate on explicit candidate-row inventories rather than only end-to-end prompt outcomes. The MSC oracle probe contains 111{,}877 candidate rows across 32 conversations; the LongMemEval probe contains 32{,}251 rows across 12 truncated conversations. This matters because the shortlist question is not ``which policy wins end to end'' in the abstract, but ``whether geometry changes the ranking inside a semantic candidate pool enough to justify deployment.''
...
\begin{table}[t]
\centering
\caption{Gate 1 on MSC valid (\pname{qwen25\_15b}, 32 conversations).}
\label{tab:msc_gate1}
\begin{tabular}{lccc}
\toprule
...
These shortlist-ranking gains are still far above the operational threshold used for Gate 1. In that sense, the oracle probe gives a clear yes/no answer: geometry is changing the ranking problem meaningfully enough inside the semantic shortlist to justify policy evaluation.

The larger-scale result sharpens the mechanism claim. Within the shortlist, query-conditioned geometry improves ranking order strongly, but does not improve top-5 recall over semantic ranking on this 32-conversation slice. The interpretation is therefore narrower and cleaner: geometry acts as a shortlist-ranking correction after semantic retrieval has already surfaced the right candidate pool.

\begin{table}[t]
\centering
\caption{MSC policy comparison from the 32-conversation scale-up. Negative deltas favor the left policy.}
...
At larger scale, the refinement claim is narrower than in the earlier checkpoint. Budget-aware semantic KCD is the strongest stable semantic baseline, beating pure semantic at budgets 0.35 and 0.50. The important point is that \emph{query-conditioned} geometry still improves over ambient geometry at budgets 0.20 and 0.35, while remaining statistically indistinguishable from budget-aware semantic. This is exactly the role we want the refinement signal to play: not as the front door, but as a shortlist-aware ranking correction.
...
\begin{table}[t]
\centering
\caption{Gate 1 on LongMemEval-S cleaned (\pname{qwen25\_15b}, 12 conversations).}
\label{tab:lme_gate1}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
...
\section{Limitations}
The strongest mechanistic evidence in the paper comes from a custom hard stress set designed to expose support-turn failures. That makes the result informative, but it also limits external validity. Although the hard set uses randomized synthetic packets constructed from memory primitives that also appear in benchmarks such as MSC, LoCoMo, and LongMemEval, it is still a controlled stress test rather than a sampled public benchmark. The public-benchmark evidence is now stronger than before on shortlist ranking because it covers 32 MSC and 12 LongMemEval conversations, but it is still stronger on oracle ranking than on broad end-to-end answer-quality coverage, especially on the truncated LongMemEval slice. The 3B validation uses only one model, so it supports only a single-model replication claim rather than a general statement about scaling. Finally, the learned harm predictor and object-level compression results are informative negatives, but their diagnoses are still based on the current implementation choices rather than exhaustive exploration.
