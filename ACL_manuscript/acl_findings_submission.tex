...
Among KCD signal variants on MSC (1{,}000 conversations, Qwen2.5-1.5B), query-conditioned geometry (\pname{sem\_query\_cond\_KCD}) is the only policy that achieves the best logit L2 \emph{and} best answer NLL simultaneously.
...
\begin{table}[t]
\centering
\caption{Component ablation of \pname{sem\_query\_cond\_KCD} on MSC (\pname{qwen25\_15b}, 1{,}000 conversations). Lower is better. Removing the support-score weighting (\textit{w/o support}) significantly degrades both metrics at all budgets ($p{<}0.001$). Removing query projection (\textit{w/o query}) hurts geometry at $B{=}0.35$ ($\Delta{=}{+}7.5$, $p{<}0.001$) but has negligible behavioral effect.}
...
Policy & L2@.20 & L2@.35 & L2@.50 & NLL@.20 & NLL@.35 & NLL@.50 \\
\midrule
\pname{sem\_query\_cond\_KCD} & \textbf{807.5} & \textbf{801.8} & \textbf{705.5} & \textbf{3.034} & \textbf{2.850} & \textbf{2.671} \\
\quad w/o query proj. & 806.7 & 809.4 & 708.3 & 3.028 & 2.856 & 2.670 \\
\quad w/o support score & 808.9 & 817.0 & 757.7 & 3.094 & 2.957 & 2.762 \\
\bottomrule
\end{tabular}}
\end{table}

At budget 0.50, \pname{sem\_query\_cond\_KCD} achieves logit L2 of 705.5---lower than uniform (761.6), meaning query-conditioned geometry is more faithful to the full-context hidden state than random turn sampling. The improvement over semantic KCD is $\Delta=-109.4$ ($p<0.001$, conversation-level). At budget 0.35, the geometry advantage is $\Delta=-12.2$ ($p<0.001$). At budget 0.20 the geometry difference is not significant ($p=0.872$), consistent with the pattern that geometry refinement is most useful when budget pressure is binding enough to force real selection decisions.

The behavioral result is equally consistent: \pname{sem\_query\_cond\_KCD} achieves the best answer NLL at every budget, with statistically significant improvements over semantic KCD at all three budgets ($p<0.001$, conversation-level). This resolves the apparent tension seen in the external baseline comparison---query-conditioned geometry does not require a geometry-behavior tradeoff; it achieves both.

Table~\ref{tab:msc_ablation} shows the component ablation. Removing the support-score weighting (\textit{w/o support}) is catastrophic: logit L2 degrades by $+52.2$ at $B{=}0.50$ and answer NLL worsens by $+0.091$ ($p<0.001$ both metrics, all budgets). The support-score term is the load-bearing component. Removing query-conditioned projection (\textit{w/o query}) causes moderate geometry degradation at $B{=}0.35$ ($+7.5$ L2, $p<0.001$) but has negligible behavioral effect ($p=0.677$), indicating the query projection contributes primarily through geometric discriminability.
