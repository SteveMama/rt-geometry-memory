...
This result is consistent with the hypothesis that geometry-based scoring requires model representations to have a particular structure: per-turn hidden-state geometry must be sufficiently discriminative for the query-conditioning signal to improve over semantic similarity. Qwen2.5-1.5B, trained with heavy emphasis on multi-turn instruction following, produces denser per-turn geometric signatures than Llama-3.2-3B, which was trained on a broader text distribution. We do not have evidence that this is due to parameter count rather than training data; we report the finding as a scope condition for the geometry signal rather than a model-size claim.
