...
\begin{abstract}
Compressing long multi-turn conversations requires deciding not only how to compress context, but which signal should rank memory candidates under a fixed budget. We study a sparse keep/compress/drop (KCD) codec and compare geometric, semantic, and hybrid scoring signals across conversational memory regimes in the current compact white-box open-model setting. The starting point is geometric characterization: conversation-state trajectories are extremely low-rank across three compact model families (mean rank95 1.049--1.528), and geometric distortion is tightly coupled to decoder drift (geometry--logit correlation 0.989--0.994), with permutation controls collapsing both effects. That structure yields a useful control signal under budget: geometry-aware allocation beats uniform retention on a custom hard stress set by rescuing memory-critical support turns more often (17/36 vs.\ 5/36 on \texttt{qwen25\_05b} at budget 0.35). Building on that result, we study a sparse keep/compress/drop codec and ask which scoring signal should drive it. On a stress-test benchmark designed to expose support-turn failures, \pname{geometry_KCD} substantially outperforms a semantic signal swap inside the same codec (pairwise $\Delta\Logit$ of +37.8 at budget 0.20, $p=0.028$; support-turn rescue 17/36 vs.\ 7/36), indicating that geometry is useful for constraint-critical memory. On public semantic-memory benchmarks, semantic shortlisting remains essential, and query-conditioned geometry improves ranking inside the shortlist at larger scale: on MSC valid (32 conversations, 111{,}877 candidate rows) the oracle gain is $\Delta$ Kendall tau $+0.35$--$0.45$, while on LongMemEval-S cleaned (12 conversations, 32{,}251 candidate rows; truncated to 40 turns) the gain is $+0.14$--$0.30$. A single-model 3B validation shows the same qualitative geometry-versus-semantics pattern on the stress set: on \texttt{qwen25\_3b}, \pname{semantic_KCD} produces $+1.13$ nats of answer NLL damage at budget 0.50 ($p=0.0003$) relative to \pname{geometry_KCD}, while geometry policies improve answer quality relative to uniform. The main limitation is that the strongest mechanistic evidence comes from a custom stress benchmark, while the public-benchmark evidence is still pilot-scale and stronger on oracle shortlist ranking than on end-to-end policy separation. The current method also assumes hidden-state access and therefore does not directly support black-box or API-only deployment. We also report two informative negatives---a learned harm predictor and memory-object level compression---and five earlier falsification results that define the claim boundary.
\end{abstract}
...
\begin{itemize}[leftmargin=1.4em]
    \item \textbf{MSC valid} (\texttt{gonced8/multi-session-chat}, 32 conversations used): multi-session dialogue with persona continuity, preference tracking, and topic flow \cite{msc}. Tests semantic-memory recall, not constraint preservation.
...
The Gate~1 threshold was $\Delta$ Kendall $\geq +0.03$. Every budget exceeds the threshold by roughly 12--15$\times$. This is still a decisive pass, but the mechanism readout is narrower than in the earlier small-slice result: inside the semantic shortlist, \pname{query_geom_v2_risk} improves ordering strongly while slightly reducing top-5 recall because the semantic shortlist already captures most obvious top-5 turns. Geometry is therefore best understood here as a shortlist-ranking correction, not as a replacement for semantic retrieval.
...
The larger-scale MSC refinement result is more conservative than the earlier 16-conversation slice. \pname{budget_aware_semantic_KCD} is now the strongest stable semantic baseline, beating pure semantic at budgets 0.35 and 0.50. Query-conditioned geometry still matters, but in a narrower way: \pname{semantic_query_conditioned_geometry_KCD} significantly beats ambient geometry at budgets 0.20 and 0.35, confirming that query projection is the operative geometric refinement, while remaining statistically indistinguishable from budget-aware semantic. The scale-up therefore supports geometry as a meaningful shortlist-refinement signal, but not yet as a benchmark-level replacement for the best budget-aware semantic codec on MSC.
...
\begin{table}[t]
    \centering
    \caption{Gate~1 oracle on LongMemEval-S cleaned (12 conversations, 32{,}251 rows). Semantic Kendall remains near-flat inside the shortlist; geometry still clears the Gate~1 threshold at every budget.}
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item Gate~1 refinement claims are now grounded in 32 MSC and 12 LongMemEval conversations, but the public-benchmark policy evidence remains narrower than the oracle-ranking evidence, especially on the truncated LongMemEval slice.
