...
\begin{abstract}
We present a manuscript-grade checkpoint of a three-stage research program on conversational memory compression for small language models, together with an explicit record of where the program succeeds and where it fails. The program proceeds from geometry measurement (Paper~1) to geometry-aware control under fixed memory budgets (Paper~2) to sparse signal-conditioned conversational memory codecs (Paper~3). In Paper~1, conversation-state trajectories are extremely low-rank across three compact model families (mean rank95 1.049--1.528) and geometric distortion is tightly coupled to decoder drift (geometry--logit correlation 0.989--0.994), with permutation controls collapsing both effects. In Paper~2, geometry-aware budget allocation beats uniform retention on a custom hard stress set by rescuing memory-critical support turns more often (17/36 vs.\ 5/36 on \texttt{qwen25\_05b} at budget 0.35). In Paper~3, we establish that the keep/compress/drop (KCD) sparse codec is a signal-agnostic compression structure whose optimal scoring depends on the memory type the task requires. Four decisive experiments close this question. First, a direct signal comparison on the hard stress set shows that \texttt{geometry\_KCD} significantly outperforms \texttt{semantic\_KCD} on constraint-critical memory (pairwise $\Delta\Logit$ of +37.8 at budget 0.20, $p=0.028$; support-turn rescue 17/36 vs.\ 7/36), confirming that the geometry signal is load-bearing and cannot be replaced by semantic scoring when exact constraint packets must be preserved. Second, a Gate~1 oracle scale-up on MSC valid (32 conversations, 111{,}877 candidate rows) shows that query-conditioned geometric features still add substantial ranking value inside a semantic shortlist ($\Delta$ Kendall tau $+0.35$--$0.45$, 12--15$\times$ the $+0.03$ threshold), while the larger refinement study shows a narrower policy result: budget-aware semantic KCD is the strongest stable semantic baseline, and query-conditioned geometry consistently beats ambient geometry rather than clearly displacing budget-aware semantic. Third, the same Gate~1 scale-up on LongMemEval-S cleaned (12 conversations, 32{,}251 candidate rows; truncated to 40 turns) passes at 5--10$\times$ threshold ($\Delta$ Kendall $+0.14$--$0.30$), confirming the cross-benchmark generality of the geometry ranking advantage even though refinement differences on the truncated slice remain weak. Fourth, a 3B model validation shows the geometry advantage grows with scale: on \texttt{qwen25\_3b}, \texttt{semantic\_KCD} produces $+1.13$ nats of answer NLL damage at budget 0.50 ($p=0.0003$) relative to \texttt{geometry\_KCD}, while every geometry policy improves answer quality relative to uniform. We also report two negative results from further investigated open items---a learned harm predictor and memory-object level compression---that define precise future work directions, and five earlier falsification results that define the claim boundary.
\end{abstract}
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item A completed Gate~1 oracle scale-up on both MSC valid ($\Delta$ Kendall $+0.35$--$0.45$ inside the semantic shortlist; 111{,}877 candidate rows) and LongMemEval-S cleaned ($\Delta$ Kendall $+0.14$--$0.30$; 32{,}251 candidate rows), confirming geometry adds substantial ranking value inside semantic shortlists across benchmark families.
...
\begin{itemize}[leftmargin=1.4em]
    \item \textbf{MSC valid} (\texttt{gonced8/multi-session-chat}, 32 conversations used): multi-session dialogue with persona continuity, preference tracking, and topic flow. Tests semantic-memory recall, not constraint preservation.
    \item \textbf{LongMemEval-S cleaned} (\texttt{xiaowu0162/longmemeval-cleaned}): longer episodic-retrieval conversations with distributed relevance (500 conversations, 398--618 turns each; truncated to first 40 turns per conversation for tractable oracle computation). Used for Gate~1 LongMemEval oracle and refinement on a 12-conversation slice (32{,}251 oracle candidate rows).
\end{itemize}
...
The Gate~1 threshold was $\Delta$ Kendall $\geq +0.03$. Every budget exceeds the threshold by roughly 12--15$\times$. This is still a decisive pass in the operational sense, but the mechanism is different from the earlier small-slice readout: inside the semantic shortlist, query-conditioned geometry improves \emph{ordering} strongly while slightly reducing top-5 recall because the semantic shortlist already captures most of the obvious top-5 turns. The larger-scale result therefore sharpens the claim: geometry is most useful here as a shortlist-ranking correction, not as a replacement for semantic retrieval.

\textbf{Gate~1 policy result.}

\begin{table}[t]
    \centering
    \caption{Gate~1 refinement study on MSC valid (32 conversations, \texttt{qwen25\_15b}). Negative $\Delta$ means the left policy is better.}
...
The larger-scale MSC refinement result is more conservative than the earlier 16-conversation slice. \texttt{budget\_aware\_semantic\_KCD} is now the strongest stable semantic baseline, beating pure semantic at budgets 0.35 and 0.50. Query-conditioned geometry still matters, but in a narrower way: \texttt{semantic\_query\_conditioned\_geometry\_KCD} significantly beats ambient geometry at budgets 0.20 and 0.35, confirming that query projection is the operative geometric refinement, while remaining statistically indistinguishable from budget-aware semantic. The scale-up therefore supports geometry as a meaningful shortlist-refinement signal, but not yet as a benchmark-level replacement for the best budget-aware semantic codec on MSC.
...
\textbf{Setup.} Oracle probe on 12 LongMemEval-S cleaned conversations (\texttt{qwen25\_15b}, conversations truncated to 40 turns each, budgets 0.20/0.35/0.50, 32{,}251 candidate rows). Refinement study comparing the same four policies as in Experiment~2.

\textbf{Gate~1 oracle result: PASS at 5--10$\times$ threshold.}

\begin{table}[t]
    \centering
    \caption{Gate~1 oracle on LongMemEval-S cleaned (12 conversations, 32{,}251 rows). Semantic Kendall is still near-flat inside the shortlist; geometry remains clearly better by the Gate~1 threshold.}
    \label{tab:gate1_lme_oracle}
    \begin{tabular}{lcccc}
        \toprule
        Budget & Semantic $\tau$ & Geometry $\tau$ & $\Delta\tau$ & Gate~1 \\
        \midrule
        0.20 & $-0.010$ & 0.304 & $+0.303$ & \checkmark\ (10$\times$) \\
        0.35 & $-0.010$ & 0.145 & $+0.145$ & \checkmark\ (5$\times$) \\
        0.50 & $-0.010$ & 0.289 & $+0.289$ & \checkmark\ (10$\times$) \\
        \bottomrule
    \end{tabular}
\end{table}

Semantic scoring remains effectively non-informative on episodic-retrieval turns inside the shortlist (Kendall near zero), while query-conditioned geometry yields positive Kendall at every budget and clears the Gate~1 threshold decisively. The overall (non-shortlist-restricted) ranking gains are larger still ($\Delta$ Kendall about $+1.16$--$+1.29$), so the cross-benchmark oracle conclusion survives the scale-up even though the shortlist-restricted gains are smaller than in the earlier 6-conversation slice.

\textbf{Refinement policy result.} The policy-side picture on the truncated LongMemEval slice is weaker than the oracle result. Query-conditioned geometry shows a row-level logit advantage over ambient geometry at budget 0.35, but that difference does not survive the conversation-level test, and answer NLL remains near-flat across policies. We therefore treat LongMemEval in this checkpoint as a replication of the oracle shortlist-ranking result, not yet as a decisive benchmark-level policy win.
...
\begin{itemize}[leftmargin=1.4em]
    ...
    \item \textbf{Persona-continuity memory (MSC valid):} geometry still adds real value inside the semantic shortlist (Gate~1 oracle $\Delta$ Kendall $+0.35$--$0.45$), and query-conditioned geometry beats ambient geometry at low-to-mid budgets. But the scale-up no longer supports a clean claim that the query-conditioned hybrid is the overall MSC policy winner; budget-aware semantic KCD is the strongest stable semantic baseline.
    \item \textbf{Episodic retrieval (LongMemEval-S cleaned):} Gate~1 still passes comfortably ($\Delta$ Kendall $+0.14$--$0.30$ inside shortlist; semantic Kendall near zero), but the truncated 40-turn refinement slice does not yield a stable benchmark-level policy winner. Query-conditioned geometry remains supported as a ranking signal, not yet as a decisive end-to-end policy result.
...
\begin{enumerate}[leftmargin=1.4em]
    ...
    \item \textbf{Geometry adds significant value inside a semantic shortlist, across benchmark families.} Gate~1 passes on MSC ($\Delta$ Kendall $+0.35$--$0.45$, 12--15$\times$ threshold) and LongMemEval ($\Delta$ Kendall $+0.14$--$0.30$, 5--10$\times$ threshold). The result is benchmark-general, not benchmark-specific, but the operative gain is ranking correction inside a semantic shortlist rather than a universal top-$k$ recall increase.
