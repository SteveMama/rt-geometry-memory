...
oracle probe on MSC valid (23{,}928 candidate rows) shows query-conditioned
geometry adds substantial ranking value inside a semantic shortlist
($\Delta\tau {+}0.53$--$0.60$, 15--20$\times$ threshold), and that
\texttt{semantic\_query\_conditioned\_geometry\_KCD} beats the strongest
semantic-only codec on answer NLL ($p{=}0.020$) at budget 0.35. Third, the
same probe on LongMemEval-S cleaned passes Gate~1 at 16--17$\times$ threshold
($\Delta\tau {+}0.49$--$0.53$), confirming cross-benchmark generality. Fourth,
...
\item \textbf{MSC valid} (\texttt{gonced8/multi-session-chat}, 16 conversations):
...
\item \textbf{LongMemEval-S cleaned} (\texttt{xiaowu0162/longmemeval-cleaned},
  6-conversation subset): episodic-retrieval conversations (398--618 turns
...
MSC valid, 16 conversations, \texttt{qwen25\_15b}, 23{,}928 candidate rows.
...
Every budget exceeds the threshold by 15--20$\times$. Geometry improves
within-shortlist turn ranking by a factor of 2--3$\times$ over semantic scoring
alone.

\paragraph{Policy result.} The refinement study compares
\texttt{semantic\_query\_conditioned\_geometry\_KCD} against
\texttt{budget\_aware\_semantic\_KCD}:

\begin{table}[t]
  \centering
  \caption{Gate 1 policy on MSC valid. Negative $\Delta$ means
  query-conditioned geometry is better. Budget 0.35 is significant on both
  metrics.}
  ...
  \begin{tabular}{lrrrr}
    ...
    0.35 & $-26.4$ & $\mathbf{0.044}$ & $-0.057$ & $\mathbf{0.020}$ \\
    0.50 & $+24.9$ & $0.450$ & $+0.008$ & $0.316$ \\
    \bottomrule
  \end{tabular}
\end{table}

At budget 0.35, the query-conditioned geometry hybrid beats the best
semantic-only codec on both logit ($p{=}0.044$) and answer NLL ($p{=}0.020$).
Query-conditioning is specifically responsible: \texttt{semantic\_ambient\_geometry\_KCD}
does not produce the same gain --- query-projected geometry beats ambient
geometry by $\Delta\Logit{=}-49.8$ ($p{=}0.004$) at budget 0.35.
...
\paragraph{Setup.} Oracle probe on 6 LongMemEval-S cleaned conversations
(40 turns per conversation, 10{,}416 candidate rows).

\paragraph{Oracle result: PASS at 16--17$\times$ threshold.}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    0.20 & 0.013 & 0.527 & $+0.514$ & \checkmark \\
    0.35 & 0.013 & 0.520 & $+0.507$ & \checkmark \\
    0.50 & 0.013 & 0.502 & $+0.489$ & \checkmark \\
    \bottomrule
  \end{tabular}
\end{table}

On episodic-retrieval conversations, semantic scoring inside the shortlist
achieves Kendall $\tau \approx 0.013$ --- essentially random. Geometry achieves
$\tau = 0.50$--$0.53$, a 40$\times$ improvement. The overall (pre-shortlist)
$\Delta\tau$ is 1.47--1.54. At budget 0.50, \texttt{semantic\_query\_conditioned\_geometry\_KCD}
beats ambient geometry by $\Delta\Logit{=}-76.6$ ($p{=}0.036$), confirming
that query-conditioned projection is necessary, not just any geometry signal.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{2.0cm}p{2.5cm}p{2.5cm}}
    ...
    Persona-continuity
      ...
      & Gate 1 MSC; $p{=}0.020$ NLL \\
    Episodic retrieval
      & Semantic shortlist + query-geom.
      & Gate 1 LME; $\Delta\tau{+}0.51$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\item Geometry adds substantial ranking value inside semantic shortlists, on
  both MSC and LongMemEval-S, at 15--23$\times$ the Gate 1 threshold. This
...
significance rests on 16 MSC and 6 LongMemEval conversations; broader
